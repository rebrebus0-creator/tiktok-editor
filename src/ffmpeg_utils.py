...
def escape_filter_value(value: str) -> str:
    """Экранирует путь для подстановки в аргумент фильтра.

    Разбор фильтрографа двухуровневый: сначала он режется по «,» и «;»,
    потом аргументы фильтра — по «:». Всё это в путях (и апостроф в имени
    файла) нужно закрыть обратным слэшем, иначе FFmpeg увидит границу
    фильтра там, где на самом деле имя файла.
    """
